...
public:
  ...
  void decompile(const Node* root, FILE *output) {
    if(root == nullptr) {
...
private:
  ...
  static void verifyTree(const Node *root) {
    if (!root) {
      return;
    }
      ...
      case NODE_DECVAR:
        if(root->symbol) {
          if(root->symbol->type != SYMBOL_IDENTIFIER) {
            fprintf(stderr,"Semantic error: variable %s already declared",root->symbol->name.c_str());
          }
        }
